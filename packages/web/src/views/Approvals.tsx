...
export function Approvals() {
  ...
  useEffect(() => {
    ...
    setSelectedRev(null);
    setContent(null);
    deliverablesApi.get(selectedId).then((raw) => {
      setDetail(raw);
      const revisions = (raw.revisions as Revision[]) ?? [];
      const currentRevId = raw.deliverable.currentRevisionId
    ...
    <div className="flex flex-1 h-screen overflow-hidden">
      ...
      <div className={`${selectedId ? 'hidden md:flex' : 'flex'} w-full md:w-[260px] md:shrink-0 border-r border-rule flex-col bg-cream`}>
        ...
        <div className="flex-1 overflow-auto">
          ...
          {pending.length === 0 && (
            <p className="text-[12px] text-ink-3 px-4 py-6 text-center">
              Nincs várakozó jóváhagyás.
            </p>
          )}
            ...
            <div className="px-5 py-4 border-b border-rule flex items-start justify-between gap-3 bg-cream shrink-0">
              ...
              {canAct && (
                <div className="flex gap-2 shrink-0">
                  ...
                  <button
                    onClick={() =>
                      void discardDeliverable(detail.deliverable.id).then(() => loadList())
                    }
                  ...
                  <button
                    onClick={() =>
                      void approveDeliverable(detail.deliverable.id).then(() => loadList())
                    }
