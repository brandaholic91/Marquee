...
export function makeSubmitDeliverableTool(ctx: SubmitDeliverableContext) {
	return {
		...
		description:
			"Add be a befejezett deliverable-t. A markdown tartalom és az opcionális structured_data automatikusan elmentődik artifact fájlként, és a deliverable jóváhagyásra vár státuszba kerül.",
		inputSchema: {
			type: "object",
			properties: {
		...
		execute: async (input: SubmitDeliverableInput) => {
			const existing = await ctx.db
				.select()
				.from(deliverables)
				.where(eq(deliverables.delegationId, ctx.delegationId))
				.all();
			const now = Date.now();
